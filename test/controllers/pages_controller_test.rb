...
class PagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test " access profile page" do
    ...
    post "/users/sign_in", params: {user: { email: users(:abc123).email,password: 'abc1231234'} }
    follow_redirect!
  ...
  test "check database insert" do
    user_test = User.find_by(email:users(:abc123).email)
    assert_equal user_test.fitbitconfigured,true
    user_test.fitbitkey = '1234'        
    user_test.save   
    assert_equal user_test.email,users(:abc123).email
    user_test_new =  User.find_by(email:users(:abc123).email)
